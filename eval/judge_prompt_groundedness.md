# Judge prompt — GROUNDEDNESS ngữ nghĩa v2
...
- **PASS (ngoại lệ out-of-scope)**: nếu `scope="out_of_scope"`, `sources=[]`, answer
  chỉ từ chối yêu cầu ngoài corpus/không tồn tại và không bịa fact, đó là groundedness
  đúng; không yêu cầu source để chứng minh việc từ chối.
...
4. **PASS (`sc-16`)**: câu hỏi thời tiết ngoài corpus, output `scope="out_of_scope"`,
   `sources=[]` và từ chối lịch sự; không fail chỉ vì không có citation.
